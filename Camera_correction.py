...
def calib(inter_corner_shape, size_per_grid, img_dir,img_type):  # 角点 ，像素大小 ，图像，类型
...
def dedistortion(inter_corner_shape, img_dir,img_type, save_dir, mat_inter, coff_dis):  # 得到畸变图像并保存畸变图像
    w,h = inter_corner_shape
    images = glob.glob(img_dir + os.sep + '**.' + img_type)
    for fname in images:
